...
public class sc_mirrorNoteScript : MonoBehaviour
{
    ...
    public GameObject Kamera, mirrorNote, noteHolder;
    ...
    void OnTriggerExit2D(Collider2D other)
    {
        ...
        if (other.tag == "Button")
        {
            ...
            Kamera.transform.position = new Vector3(Kamera.transform.position.x, Kamera.transform.position.y, Kamera.transform.position.z * -1f);
                    if(noteHolder.GetComponent<sc_BeatScroller>().isMirrored == false)
                    {
                        ...
                        noteHolder.GetComponent<sc_BeatScroller>().isMirrored = true;
                    } else 
                    {
                        Kamera.transform.rotation = Quaternion.Euler(0, 0, 0);

                        noteHolder.GetComponent<sc_BeatScroller>().isMirrored = false;
                    }
